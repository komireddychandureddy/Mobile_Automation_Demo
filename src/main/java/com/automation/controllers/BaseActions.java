...
public class BaseActions
{
	...
	public Actions ac;
	public Alert al;
	public AppiumDriver app_driver;
	...
	{
		((JavascriptExecutor) app_driver).executeScript("scroll(0, -100);");
	}
	...
	{
		return app_driver.findElement(locator);
	}
	...
	public boolean isWebElementNotPresent1(By locator,String logstep) {
		boolean status=app_driver.findElement(locator).isDisplayed();
		if (!status) {
		 ...
		 * element=app_driver.findElement(locator); JavascriptExecutor
		 * js=(JavascriptExecutor)app_driver;
		 * 
		 * js.
		...
		public void highLightElement( By locator) {
			WebElement element=app_driver.findElement(locator);
		    for (int i = 0; i < 2; i++) {
		        JavascriptExecutor js = (JavascriptExecutor) app_driver;
		...
		public boolean  waitUntilElement(By type, int timeOut) {
			...
			while(time<timeOut)
			{
				status=app_driver.findElements(type).size()>0;
				if(status) {
					status =true;
		...
		public WebElement waitForClickable(By locator) {
			WebDriverWait wait = new WebDriverWait(app_driver, DEFAULT_WAIT_SECONDS);
		...
		public void hoverElement(By locator) {
			...
			Actions builder = new Actions(app_driver);
		...
		public boolean doubleClick(By locator) {
			...
			try {
				...
				WebElement element = app_driver.findElement(locator);
				Actions action = new Actions(app_driver).doubleClick(element);
		...
		public boolean switchToFrame(String frameName)
		{
			...
			try
			{
				app_driver.switchTo().frame(frameName);
				return true;
			}
			catch(Exception e)
			{
		...
		public boolean switchToFrame(By locator)
		{
			...
			try
			{
				app_driver.switchTo().frame(app_driver.findElement(locator));
				return true;
			}
			catch(Exception e)
			{
		...
		public boolean switchTodefaultContent()
		{
			try
			{
				app_driver.switchTo().defaultContent();
		...
		public boolean clickAndHold(By locator1, By locator2  ) {
			...
			//	LogUtil.infoLog(KeywordUtil.class, KeywordUtil.lastAction);
				WebElement element1 = app_driver.findElement(locator1);
				WebElement element2 = app_driver.findElement(locator2);
				Actions action = new Actions(app_driver);
		...
		public boolean rightClickJS(By locator1) {
			boolean result=false;
			try {
				WebElement element = app_driver.findElement(locator1);
				JavascriptExecutor js = (JavascriptExecutor) app_driver;
		...
		public boolean rightClick1(By locator1) {
			...
			try {
				WebElement element = app_driver.findElement(locator1);
				Actions oAction=new Actions(app_driver);
		...
		public int getCoordinates(By locator) throws Exception {
			  //Locate element for which you wants to retrieve x y coordinates.
			       WebElement Image = app_driver.findElement(locator);
	...
	public String getTenDaysDate(String dateformat) {
		...
		public boolean waitForElement(By locator, int maxtime) {
	        int time=0;
	         boolean status=false;
	         while (time < maxtime) {       	 
	        	 status=app_driver.findElements(locator) .size()>0;
	        	 if(status) {
	        		 status=true;   
	        		 break;
	        	 }
		...
		public  String alertText() {
			String text= app_driver.switchTo().alert().getText();
		...
		public  void scrollTo(By locator) {
	        ...
	        ((JavascriptExecutor) app_driver).executeScript(
	                "arguments[0].scrollIntoView();", app_driver.findElement(locator));
	    }
		public  void scrollToBottom() {
	        ((JavascriptExecutor) app_driver)
	                .executeScript("window.scrollTo(0, document.body.scrollHeight)");
	    }
		
		public void scrollingByCoordinatesofAPage(int x, int y) {
			((JavascriptExecutor) app_driver).executeScript("window.scrollBy("+x+","+y+")", "");
		}
		
		public void scrollingByCoordinatesofAPage1(int x, int y) {
			((JavascriptExecutor) app_driver).executeScript("scroll("+x+","+y+");");
		}
	/* To click a certain Web Element using DOM/ JavaScript Executor */
	public void JSclick(WebElement element) 
	{
		((JavascriptExecutor) app_driver).executeScript("return arguments[0].click();", element);
	}
	...
	{
		app_driver.switchTo().frame(index);
	}
	...
	{
		app_driver.switchTo().frame(element);
	}
	...
	{
		ArrayList<String> tabs = new ArrayList<String> (app_driver.getWindowHandles());
		for(int i=1;i<tabs.size();i++)
		{	
			app_driver.switchTo().window(tabs.get(i));
			app_driver.close();
		}
		app_driver.switchTo().window(tabs.get(0));
	}
	...
	{
		ArrayList<String> al = new ArrayList<String>(app_driver.getWindowHandles());
	...
	public void navigateToUrl(String url) {                                          
		...
		app_driver.navigate().to(url);
	}

	public String getCurrentUrl() {
		return app_driver.getCurrentUrl();
	}
		 ...
		 * @throws IOException
		 */
		public String takeScreenshotAshot(AppiumDriver driver, String testCaseID) throws IOException {
		...
		public String takeScreenshot(AppiumDriver driver, String testCaseID)  {
			Date date = new Date();
			DateFormat dateFormat = new SimpleDateFormat("yyyyMMddHHmmss");
			String timeStamp = dateFormat.format(date);
			timeStamp=timeStamp+"_";			
		...
		public String takeScreenshotWebElement(WebElement element, String testCaseID) throws IOException {
			...
			Screenshot screenshot = new AShot().
					...
					takeScreenshot(app_driver, element);
			File src = new File(path);
			LogUtil.infoLog(BaseActions.class, "Screenshot image path: " + src.getPath());
			ImageIO.write(screenshot.getImage(), "PNG", src);
			return ConfigReader.getValue("screenshotPath") + "\\" +timeStamp+ testCaseID + ".jpg";
